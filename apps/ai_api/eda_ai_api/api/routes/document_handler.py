...
from eda_ai_api.models.document_handler import DocumentUploadResponse, DocumentSearchResponse
        ...
        if file.content_type not in ALLOWED_DOCUMENT_TYPES:
            ...
            if user_platform_id:
                await memory.add_message_to_history(
                    ...
                    platform="whatsapp",
                    metadata={"event_type": "document_upload", "status": "failed"}
                )
...
async def cleanup_expired_documents():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error cleaning up documents: {str(e)}"
        )