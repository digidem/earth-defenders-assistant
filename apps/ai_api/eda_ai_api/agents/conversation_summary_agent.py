...
def get_conversation_summary_agent(session_id: str):
    """
    ...
    tools = [
        ...
        GlobalKnowledgeSearchTool(),
        ConversationMemoryTool(session_id=session_id),
    ]

    agent = CodeAgent(
        name="conversation_summary_agent",
        description="Agent specialized in creating short summaries about the user's conversation. "
                    "It can retrieve conversation history by date, search global knowledge, and perform semantic memory search.",
        tools=tools,
        model=model,
        max_steps=3,
    )
